...
async function handlePairingCode(sock, device, supabase, readyToRequest, pairingCodeRequested, retryCount = 0) {
  ...
  if (pairingCodeRequested) {
    ...
    if (timeSinceRequest < 50) {
      console.log('⏱️ Pairing code still fresh, skipping request');
      return { handled: false }; // Still fresh, don't request again
    }
    console.log('⏰ Previous pairing code expired, generating new one...');
  }
    ...
    // Longer delay to ensure WhatsApp connection is fully ready
    // This prevents 428 errors
    ...
    try {
      ...
      // Check if error is retryable (428, 401, or connection issues)
      const isRetryableError = status === 428 || 
                               status === 401 ||
                               status === 503 ||
                               /precondition/i.test(pairErr?.message) ||
                               /unauthorized/i.test(pairErr?.message) ||
                               /connection closed/i.test(pairErr?.message) ||
                               /connection failure/i.test(pairErr?.message);
